...
protected:
    ...
    struct TreeNode {
        ...
        int tam_i; //num de nodos en hijo izq + 1
...
public:
    ...
    T kesimo(int k) const {
        if (k < 1 || k > nelems) {
            throw std::runtime_error("??");
        }

        return kesimoP(raiz, k);
    }
...
protected:
    ...
    T kesimoP(Link a, int k) const {
        if (a == nullptr) {
            throw std::runtime_error("??");
        }

        int tamizq = (a->iz != nullptr) ? a->iz->tam_i : 0;

        if (k == tamizq + 1) 
            return a->elem;
        else if (k <= tamizq)
            return kesimoP(a->iz, k);
        else
            return kesimoP(a->dr, k - tamizq - 1);
    }
