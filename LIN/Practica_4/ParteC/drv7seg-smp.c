...
static int display7s_open (struct inode *i, struct file *filp){
	...
	//printk(KERN_INFO "OPEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEn \n" );
...
static int display7s_release (struct inode *i, struct file *filp){
	...
	//printk(KERN_INFO "RELEASEEEEEEEEEEEEEEEEEEEEEEEEE \n" );
...
static ssize_t display7s_write(struct file *filp, const char *buff, size_t len, loff_t *off)
{
     ...
     if (len > 2)
     {
          pr_err("Se esperaba un solo caracter, pero se proporcionaron %zu caracteres\n", len);
          return -EINVAL;
     }
    ...
    {
        pr_err("Fallo al copiar datos desde el espacio de usuario\n");
        return -EFAULT;
    }

    const unsigned char hex_values[16] = {
        0xFC, 0x60, 0xDA, 0xF2,
        0x66, 0xB6, 0xBE, 0xE0,
        0xFE, 0xF6, 0xEE, 0x3E,
        0x9C, 0x7A, 0x9E, 0x8E
    ...
    if (sscanf(&digit,"%x \n" ,&display_value) == 1)
    {
        ...
        if(display_value < 16){
			...
			printk(KERN_INFO "LEEEENmak%x \n", len );

    		update_7sdisplay(hex_values[display_value]);
		}
		else
		{
			pr_err("Carácter no válido: %c\n", digit);
			return -EINVAL;
		}
    }
    else
